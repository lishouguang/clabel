...
def extract_from_txt(txt):
    ...
    labels = []
    for txt in txts:
        for sent in parser.parse2sents(utils.unicode2str(txt)):
...
def _extract_labels_stem(sentence):
    ...
    rrelation = sentence.find_relations('HED')

    # 声音小
    # 核心词是形容词/其他名词修饰符
    if rrelation.token2.pos in ['a', 'b']:
        # SBV关系为标签主干
        sbv = sentence.find_relations('SBV')
        if sbv:
            save_labels(labels, features, opinions, sbv.token2, sbv.token1)
        else:
            save_labels(labels, features, opinions, None, sbv.token1)

    # 喇叭是坏的
    # 核心词是动词
    elif rrelation.token2.pos == 'v':
    # else:
        # SBV + VOB构成标签主干
        sbv = sentence.find_relations('SBV')
        vob = sentence.find_relations('VOB')
        if sbv and vob:
            save_labels(labels, features, opinions, sbv.token2, vob.token2)

        else:
            # CMP构成标签主干
            cmp_ = sentence.find_relations('CMP', token2_pos='a')
            if cmp_:
                save_labels(labels, features, opinions, rrelation.token2, cmp_.token2)
